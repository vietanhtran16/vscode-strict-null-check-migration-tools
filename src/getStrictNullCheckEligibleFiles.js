...
const forEachFileInSrc = (srcRoot, options) => {
    return new Promise((resolve, reject) => {
        glob(`${srcRoot}/**/*.{ts,tsx}`, { nodir: true, silent: true }, (err, files) => {
            if (err) {
                return reject(err);
            }
            return resolve(files.filter(file =>
                !file.endsWith('.d.ts')
                && !file.endsWith('.d.tsx')
                && (options && options.includeTests ? true : (!file.endsWith('.test.tsx') && !file.endsWith('.test.ts')))));
        })
    });
};
...
async function getCheckedFiles(tsconfig, srcRoot) {
    ...
    const includes = tsconfig.include.map(include => {
        return new Promise((resolve, reject) => {
            glob(path.join(srcRoot, include), { nodir: true, silent: true }, (err, files) => {
                if (err) {
                    return reject(err);
                }
